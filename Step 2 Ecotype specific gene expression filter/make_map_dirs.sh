...
#Expect a folder 'reference' that contains the polished Col-0 and Tsu-1 reference transcriptomes from step 1
...
for DD in `seq 0 11` ; do
    ...
    cd $DIR2
    ln -s ../reference/* .
    ln -s /Trimmed/Reference_2/*${SAMPLE}*.fq.gz . #Location to trimmed reads
    cd ..
done

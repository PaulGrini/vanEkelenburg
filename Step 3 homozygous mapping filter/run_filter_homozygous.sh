...
python3 ${SRC}/filter_homozygous.py Early.model.tsv    >    Early.min_and_fold.tsv
python3 ${SRC}/filter_homozygous.py Late.model.tsv     >     Late.min_and_fold.tsv

echo "Write genes to *.genes_pass_filter ..."
echo "Filter for min fold change 5"
# Use c to skip the header line

cat    Early.min_and_fold.tsv | awk '{if ((c++ >0) && ($4>=5)) print $1;}' >    Early.genes_pass_filter
cat     Late.min_and_fold.tsv | awk '{if ((c++ >0) && ($4>=5)) print $1;}' >     Late.genes_pass_filter
